<x-app-layout>
    <x-slot name="title">New Audit</x-slot>

    <x-page-header
        title="New Audit"
        subtitle="Record the result of a physical audit for an asset." />
    ...
    <form method="POST" action="{{ route('audits.store') }}" enctype="multipart/form-data" class="max-w-3xl">
        @csrf
        ...
        <x-card title="Audit Details">
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <select id="asset_id" name="asset_id" required class="input-field">
                        ...
                        @foreach ($assets as $asset)
                            <option value="{{ $asset->id }}" @selected(old('asset_id') == $asset->id)>
                                {{ $asset->asset_code }} — {{ $asset->asset_name }} ({{ $asset->location?->name ?? 'No location' }})
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="audit_date" name="audit_date" value="{{ old('audit_date', now()->toDateString()) }}" required class="input-field">
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="condition" name="condition" value="{{ old('condition') }}" class="input-field" placeholder="e.g. Good, Fair, Damaged">
                </div>
                ...
                <div>
                    ...
                    <select id="location_match" name="location_match" class="input-field">
                        <option value="1" @selected(old('location_match', '1') === '1')>Yes — asset found at recorded location</option>
                        <option value="0" @selected(old('location_match') === '0')>No — asset was elsewhere</option>
                    </select>
                </div>

                <div class="md:col-span-2">
                    <label for="findings" class="label-field">Findings</label>
                    <textarea id="findings" name="findings" rows="3" class="input-field" placeholder="Notes from the audit...">{{ old('findings') }}</textarea>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="mt-1 text-body-sm text-on-surface-variant">Optional. JPEG, PNG, GIF, WebP up to 5MB.</p>
                </div>
            </div>
        </x-card>
        ...
        <div class="mt-6 flex items-center gap-3">
            ...
            <a href="{{ route('audits.index') }}" class="btn-secondary">Cancel</a>
        </div>
    </form>
</x-app-layout>
